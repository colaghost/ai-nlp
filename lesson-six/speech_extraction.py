...
def locate_speech_entity_for_sentence(sentence, postagger, parser):
    talk_words = set('留言 说 怒道 质问 曾言 喊道 写道 笑答 地问 脱口而出 答 大喊 哭道 直言 告诫 表示 痛骂 破口大骂 大叫 著说 时说 坦言 怒斥 辩称 问说 责备 询问 唱道 中说 宣称 公曰 追问 还称 有言 喃喃自语 责骂 斥责 答说 慨叹 地说 谈到 问过 臣闻 坚称 说出 坦承 评说 韦昭注 叮嘱 自言 认为 信中称 奏道 诗云 惊呼 不禁 提到 宣宗称 文说 评曰 透露 所说 请问 自谓 怒骂 谚云 大呼 否认 骂 问起 指责 痛斥 惊道 劝道 指称 称 喝道 中称 言道 公说 相信 如是说 强调 说明 注云 重申 表明 辱骂 介绍'.split())
    ...
    for i in range(len(words)):
        if relation[i] == 'SBV' and (heads[i] in talk_words):
            ...
            while last_talk_word_id + 1 < len(words):
                if rely_id[last_talk_word_id + 1] - 1 == last_talk_word_id and relation[last_talk_word_id + 1] == 'COO':
                    last_talk_word_id += 1
            ...
            if last_talk_word_id + 1 < len(words):
                return (entity, ''.join(words[last_talk_word_id+1:]))
    return None

def split_senteces(content):
